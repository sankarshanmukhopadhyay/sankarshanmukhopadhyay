---
layout: home
title: Trust Infrastructure Portfolio
---

# Trust Infrastructure Portfolio

This site publishes the governance and adoption control surface for Sankarshan Mukhopadhyay's public trust-infrastructure work.

## Start here

- [Portfolio governance](../portfolio/README.md)
- [Architecture and authority boundaries](../portfolio/architecture.md)
- [Adoption readiness gate](../portfolio/adoption-checklist.md)
- [Drift review procedure](../portfolio/drift-review.md)
- [Portfolio Assurance Monitor](portfolio-assurance/index.md)
- [Portfolio assurance dashboard](portfolio-assurance/dashboard.md)
- [Repository status registry](../data/repository-status.yaml)
- [Relationship registry](../data/portfolio-relationships.yaml)

## Assurance claim

Portfolio status is not self-certifying. The registries state intended authority and lifecycle; repository-level tests and release evidence remain authoritative for implementation claims.
